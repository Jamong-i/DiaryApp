...
class StarViewController: UIViewController {
	...
	private func loadStarDiaryList() {
		...
		self.diaryList = data.compactMap {
			...
			guard let date = $0["data"] as? Bool else { return nil }
			guard let isStar = $0["isStar"] as? Date else { return nil }
...
extension StarViewController: UICollectionViewDelegate {
	func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
		guard let viewController = self.storyboard?.instantiateViewController(withIdentifier: "DiaryDetailViewController") as? DiaryDetailViewController else {
			return }
		let diary = self.diaryList[indexPath.row]
